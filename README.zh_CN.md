...
| `OK` | 长按 | 保存并退出,返回上一级(菜单中:进入选中项) |
| `UP` | 短按 | 静音 / 取消静音(菜单中:上移选中项) |
| `DOWN` | 短按 | 循环切换音色 CLASSIC / BELL / BLOCK / DROP(菜单中:下移选中项) |
| `DOWN` | 长按 | 每 0.7 秒自动敲击 |
| `电源键` | — | 硬件开关:直接通断电源,不经固件 |

> **实机电源键为硬件开关**,直接通断电源、固件不可见,开机和关机都由它完成;
> 木鱼页 2 分钟无操作自动息屏,任意键亮屏。固件保留了独立电源键的软件接口
> (`bsp_pins.h` 的 `BSP_PWR_BTN_GPIO`):填入引脚后短按=息屏/亮屏、长按 1.5 秒
> =任何页面关机(深睡,按 `UP` 唤醒)。注意本板 GPIO 已全部占用,GPIO11~17 还
> 接着片上 Flash(配置即死机),保持 `-1` 即可;自制板若把电源键挂在按键分压链
> 上,进 Button 页按住它观察 ADC 电压读数,扩 `BSP_BTN_MV_TABLE` 为 4 档启用。
